<template>
    <ion-app style="padding-top: 55px">
        <ion-content v-if="datas.data.length > 0">
        ...
        <div  v-else style="width: 100vw; height: 100vh; display: flex; align-items: center; justify-content: center; font-size: 150%">
            Aucun résultats
        </div>
        <GoBack />
    ...
    export default {
        ...
        methods:{
            getAllProducts(page){
                ...
                let url = `http://sneakers-x.herokuapp.com/api/products?page=${page}&search=${this.$route.params.search}`
                axios.get(url)
                    .then(res => {
                        {
